...
class SimpleDQN(object):
    ...
    def init_model(self, random_seed):
        ...
        self._model["W1"] = np.random.randn(self._D, self._H) / np.sqrt(
            self._D
        ...
        # time.sleep(5)
        self._grad_buffer = {
            k: np.zeros_like(v) for k, v in self._model.items()
    ...
    def update_parameters(self):
        for k, v in self._model.items():
            ...
            self._rmsprop_cache[k] = (
                self._decay_rate * self._rmsprop_cache[k]
                + (1 - self._decay_rate) * g ** 2
            )
            self._model[k] -= (
                self._learning_rate * g / (np.sqrt(self._rmsprop_cache[k]) + 1e-5)
            )
